const { repository } = require('../models/index')
...
exports.findOne = (req, res) =>
{
    try 
    {
        ...
        if (ID)
        {
            repository.findByPk(ID).then(async data =>
            {
                if (data) 
                    return res.send(data)
...
exports.Create = (req, res) =>
{
    ...
    {        
        ...
        {
            res.send({ message: 'repository created successfully!' })
        }).catch((error) => 
        {
            return res.status(400).send({ message: 'repository data invalid or missing.' })   
        })
    }
    catch (error)
    {
...
exports.Update = async (req, res) =>
{
    try 
    {
        ...
        repository.update(req.body, { where: { id: ID } }).then((num) => 
        {
...
exports.Delete = (req, res) =>
{
    try 
    {
        ...
        repository.destroy({ where: { id: ID } }).then((num) => 
        {
            const status = num > 0 ? 200 : 400
